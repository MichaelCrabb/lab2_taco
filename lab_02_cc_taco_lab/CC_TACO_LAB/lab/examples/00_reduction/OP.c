...
{
  op_inouts->num_elem_z_scalar = 1;

  // Initialize the buffer
  op_inouts->z_scalar = (float *)malloc(sizeof(float)*op_inouts->num_elem_z_scalar);

  // Fill the buffer with data
  fill_buffer_with_random( op_inouts->num_elem_z_scalar, op_inouts->z_scalar);
}
...
{
  op_inouts_dst->num_elem_z_scalar = op_inouts_src->num_elem_z_scalar;

  memcpy(op_inouts_src->z_scalar,
	 op_inouts_dst->z_scalar,
	 op_inouts_src->num_elem_z_scalar*sizeof(float));
...
{
  float *output_diffs = (float *)malloc(op_inouts_ref->num_elem_z_scalar*sizeof(float));

  // This will compute the error in between the strides of the values.
  float res = compute_pair_wise_diff(op_inouts_ref->num_elem_z_scalar,1,1,1,
				     op_inouts_ref->z_scalar,
				     op_inouts_tst->z_scalar,
				     output_diffs);
  long counts = count_num_errors(op_inouts_ref->num_elem_z_scalar,1,1,1,
				 output_diffs);
