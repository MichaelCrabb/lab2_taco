...
void op_inputs_init(op_params_t *op_params, op_inputs_t *op_inputs)
{
  ...
  */
  op_inputs->num_elem_A_mat =
    ...
    max(op_inputs->n0,op_inputs->cs_a)*
    min(op_inputs->rs_a,op_inputs->cs_a);

  op_inputs->num_elem_x_vect =
    op_inputs->n0;
...
void op_inouts_init(op_params_t *op_params, op_inouts_t *op_inouts)
{
  ...
  op_inouts->num_elem_y_vect =
    op_params->m0;

  // Initialize the buffer
  op_inouts->y_vect = (float *)malloc(sizeof(float)*op_inouts->num_elem_y_vect);

  // Fill the buffer with data
  fill_buffer_with_random( op_inouts->num_elem_y_vect, op_inouts->y_vect);
}
...
void op_inouts_copy_deep(op_inouts_t *op_inouts_src, op_inouts_t *op_inouts_dst)
{
  ...
  op_inouts_dst->num_elem_y_vect = op_inouts_src->num_elem_y_vect;

  memcpy(op_inouts_src->y_vect,
	 op_inouts_dst->y_vect,
	 op_inouts_src->num_elem_y_vect*sizeof(float));
...
{
  float *output_diffs = (float *)malloc(op_inouts_ref->num_elem_y_vect*sizeof(float));

  // This will compute the error in between the strides of the values.
  float res = compute_pair_wise_diff(op_inouts_ref->num_elem_y_vect,1,1,1,
				     op_inouts_ref->y_vect,
				     op_inouts_tst->y_vect,
				     output_diffs);
  long counts = count_num_errors(op_inouts_ref->num_elem_y_vect,1,1,1,
				 output_diffs);
				...
				hwctx_t      *hw_ctx )
{
  ...
  // "size,m,n,rs_a,cs_a,flop,throughput,bytes,GB_per_s,nanoseconds\n");
...
void bench_config_init( int argc, char *argv[], benchmark_configuration_t *bench_config)
{
  ...
  if(argc == 1 )
  ...
  else if(argc == 7 + 1 || argc == 8 + 1 )
    {
      ...
      bench_config->in_rs_a=atoi(argv[6]);
      bench_config->in_cs_a=atoi(argv[7]);
  ...
  else
    {
      ...
      printf("usage: %s min max step m0 n0 rsa csa [filename]\n",
	     argv[0]);
      exit(1);
    }
