...
{
  // Note: M and N are flipped relative to A
  op_inouts->m0   = op_params->n0;
  op_inouts->n0   = op_params->m0;
				...
				hwctx_t      *hw_ctx )
{
  ...
  // "size,m,n,rs_a,cs_a,rs_c,cs_c,flop,throughput,bytes,GB_per_s,nanoseconds\n");
...
void bench_config_init( int argc, char *argv[], benchmark_configuration_t *bench_config)
{
  ...
  if(argc == 1 )
  ...
  else if(argc == 10 + 1 || argc == 11 + 1 )
    {
      ...
      bench_config->in_rs_c=atoi(argv[8]);
      bench_config->in_cs_c=atoi(argv[9]);
      ...
      if(argc == 11 + 1)
	bench_config->result_file = fopen(argv[11],"w");

    }
  else
    {
      //        argv 0   1    2    3  4  5   7   8   9  10        11
      printf("usage: %s min max step m0 n0 rsa csa rsc csc [filename]\n",
	     argv[0]);
      exit(1);
    }
